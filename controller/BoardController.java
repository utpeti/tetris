...
public class BoardController {
    ...
    public void gameAction() {
        if (isFallingFinished) {
            ...
            newPiece();
            try {
                AudioInputStream audioIn = AudioSystem.getAudioInputStream(new File("sound/tetris_drop.wav"));
    ...
    public void start() {
        ...
        timer.start();
        try {
            AudioInputStream audioIn = AudioSystem.getAudioInputStream(new File("sound/tetris_theme.wav"));
            Clip clip = AudioSystem.getClip();
            clip.open(audioIn);
            clip.start();
    ...
    public void paint(Graphics g, double width, double height) {
        ...
        for (int i = 0; i < boardHeight; ++i) {
            for (int j = 0; j < boardWidth; ++j) {
                model.Shape.Tetrominoes shape = shapeAt(j, boardHeight - i - 1);
                if (shape != model.Shape.Tetrominoes.NoShape)
                    tetrisBoard.drawSquare(g, j * squareWidth,
                            boardTop + i * squareHeight, shape);
            }
        }

        if (currentPiece.getPieceShape() != model.Shape.Tetrominoes.NoShape) {
            for (int i = 0; i < 4; ++i) {
